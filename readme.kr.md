...
이 테스트는 적은 투자로 큰 가치를 제공하는 친절한 조수인 부조종사를 만날 수 있는 기회입니다. 과학에 따르면 우리 뇌에는 두 가지 시스템이 있는데, 시스템 1은 텅 빈 도로에서 자동차를 운전하는 것과 같은 쉬운 활동에 사용되고 시스템 2는 수학 방정식을 푸는 것과 같은 복잡하고 의식적인 작업을 위한 것입니다. 시스템 1을 위한 테스트를 설계할 때는 테스트 코드를 볼 때 2X(17 × 24) = 4.6을 푸는 것이 아니라 HTML 문서를 수정하는 것처럼 쉽게 느껴져야 합니다.
