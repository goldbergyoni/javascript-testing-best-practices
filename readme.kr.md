...
:white_check_mark: **Guide:** 본 테스트 코드는 실제로 사용되는 코드와 다릅니다. 단순하고, 짧고, 이해하기 편리하게 디자인 하세요. 누구나 코드를 보고 즉시 그 의미를 알아챌 수 있어야합니다.

우리들의 뇌는 실제로 사용되는 코드와 관련된 많은 양의 부가적이고 복잡한 부분들을 생각할 여유가 없습니다. 또, 어려운 코드를 억지로 생각해내려고 한다면, 코드를 생각해내는 속도를 늦추게 되어 본 글을 읽는 이유가 무색해질 것입니다. 실제로 많은 시도자들이 이런 이유로 테스트를 포기합니다.

테스트는 친절한 동료와 함께 일하는 것이 즐거울 수 있는 기회이고, 적은 투자로 큰 열매를 얻을 수 있는 기회입니다. 과학은 우리에게 두 개의 뇌 시스템이 있다고 말합니다. 첫번째 시스템은 빈 도로에서 자동차를 운전하는 등의 단순한 행동에 사용되는 시스템이고 두번째는 수학 방정식을 푸는 것과 같이 복잡하고 어려운 연산을 위한 시스템입니다. 테스트 코드를 볼 때 어려운 수학 문제를 푸는것이 아닌, HTML 문서를 수정하는 것만큼 쉽게 첫번째 시스템에 맞게 코드를 작성하세요.

[//]: <> (체리픽 기술과 도구, 그리고 효율적인 ROI를 제공하는 테스트 코드 선정으로 이러한 목적을 달성할 수 있습니다. 필요한 만큼의 테스트, 융통성 있게 유지하려는 노력, 때로는 애자일함과 단순성을 위해 일부 테스트와 신뢰성을 포기하는 것도 가치가 있습니다: I'll remove this part to enhance understanding.)

![alt text](/assets/headspace.png "우리 머리속은 부가적인 복잡한 것들을 생각할 여유가 없습니다.")
 
앞으로 나올 대부분의 조언은 위 원칙에 따랐습니다.
...
:white_check_mark: **Guide:** 테스트는 현재 애플리케이션의 개정판이 요구 사항을 충족하는지 여부를 배포를 하는 사람과 DevOps 엔지니어, 그리고 프로그래밍 입문자에게 알려야합니다. 테스트가 요구 사항을 만족하고 아래와 같이 세 부분으로 구성되어 있다면, 목적을 이룰 수 있습니다.

(1) 무엇을 테스트하고 있는가? ex) 인터넷 쇼핑몰 - 새로운 제품을 추가하는 `ProductsService.addNewProduct` 메소드

(2) 어떤 상황과 시나리오에서? ex) 메소드에 가격이 전달되지 않는다.

(3) 예상되는 결과는 무엇인가? ex) 신제품은 승인되지 않는다.

<br/>

❌ **요구사항을 만족하지 않았을시:** 배포에 실패하고 "제품 추가" 라는 테스트에 실패하였다. 이것이 정확히 어떤 오작동인지를 알려주나요?

<br/>

**👇 Note:** 클릭하면 각 글에 있는 예제 코드와 이미지들을 볼 수 있습니다.

<details><summary>✏ <b>예제 코드</b></summary>
  
<br/>
  
### :clap: 올바른 예: 세 부분으로 구성된 테스트
